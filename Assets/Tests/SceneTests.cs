...
{
    public const float k_Threshold = 30.0f;

    static float InvertedThreshold
    {
        get
        {
            return 360.0f - k_Threshold;
        }
    }

    static float InvertedDoubleThreshold
    {
        get
        {
            return 360.0f - 2.0f * k_Threshold;
        }
    }
    ...
    public static bool GreaterThan(float a, float b)
    {
        ...
        bool crossed360 = b < k_Threshold && a > InvertedThreshold || b > InvertedThreshold && a < k_Threshold;
        return crossed360 ? (b - a) > InvertedDoubleThreshold : a > b;
    }
};
...
public class SceneTests : InputTestFixture
{
    ...
    public IEnumerator LeftRotateTest([ValueSource("dummyData")] int _)
    {
        ...
        Assert.IsTrue(Angle.GreaterThan(postMoveRotation.z, preMoveRotation.z), "Rotation around Z axis did not occur, or went backwards.");
    }
    ...
    public IEnumerator RightRotateTest([ValueSource("dummyData")] int _)
    {
        ...
        Assert.That(preMoveRotation.x, Is.EqualTo(postMoveRotation.x), "Rotation occurred around X axis, but should only occur around Z axis.");
        Assert.That(preMoveRotation.y, Is.EqualTo(postMoveRotation.y), "Rotation occurred around Y axis, but should only occur around Z axis.");
        Assert.IsTrue(Angle.GreaterThan(preMoveRotation.z, postMoveRotation.z), "Rotation around Z axis did not occur, or went backwards.");
    }
